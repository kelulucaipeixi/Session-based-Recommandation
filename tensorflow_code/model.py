...
class Model(object):
    ...
    def forward(self, re_embedding, train=True):
        ...
        if not self.nonhybrid:
            ma = tf.concat([tf.reduce_sum(tf.reshape(coef, [self.batch_size, -1, 1]) * seq_h, 1),
                            tf.reshape(last_h, [-1, self.out_size])], -1)
    ...
    def run(self, fetches, tar, item, adj_in, adj_out, alias, mask):
        self.counter+=1
        results=self.sess.run(fetches, feed_dict={self.tar: tar, self.item: item, self.adj_in: adj_in,
                                                 self.adj_out: adj_out, self.alias: alias, self.mask: mask})
        if self.counter%8000==0:
            self.saver.save(self.sess,'./checkpoint_dir/sampleModel',global_step=self.counter)
        return results
...
class GGNN(Model):
    def __init__(self,hidden_size=100, out_size=100, batch_size=300, n_node=None,
        ...
        #self.saver=tf.train.import_meta_graph('./checkpoint_dir/sampleModel-400.meta')
        #self.saver.restore(self.sess,'./checkpoint_dir/sampleModel-400')
