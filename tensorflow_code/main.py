...
def my_test(test_data):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        # if j>=20:
        #     print('Recall@20:%.4fMMR@20:%.4f'%(np.mean(hit)*100,np.mean(mrr)*100))
        #     return
    hit = np.mean(hit)*100
    mrr = np.mean(mrr)*100
    ...
    if mrr >= best_result[1]:
        best_result[1] = mrr
        best_epoch[1]=epoch
    return test_loss,best_result[0],best_result[1]
    # print('test_loss:\t%.4f\tRecall@20:\t%.4f\tMMR@20:\t%.4f\tEpoch:\t%d,\t%d'%
    #       (test_loss, best_result[0], best_result[1], best_epoch[0], best_epoch[1]))  
...
for epoch in range(opt.epoch):
    ...
    for i, j in zip(slices, np.arange(len(slices))):
        ...
        #     my_test(test_data)
    loss = np.mean(loss_)
    test_loss,Recall_20,MRR_20=my_test(test_data)
    print('train_loss\t%.4f\ttest_loss\t%.4f\tRecall@20\t%.4f\tMRR@20\t%.4f\t'%(loss,test_loss,Recall_20,MRR_20))
